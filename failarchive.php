<?php
header('Content-Disposition: inline; filename="failarchive.php"');
...
echo '	<input type="text" name="keyword" class="text" size="12"  maxlength="30" placehfailer="Search query" onFocus=" if (value == ';
echo "''";
...
//		echo "<td>";
		...
		if( (time() - filemtime("fail/$file")) <= ( $showdays * 86400 ) )
		{
			//the Output of the dox
                        echo '<tr>';
                        echo '<td>';
			echo '<a href="failviewer.php?fail='.$xfile.'" alt="'.$xfile.'">'.$xfile."</a>";
...
//			echo '</td>';
			...
			echo '<td>';
				echo date("m/d/Y", filemtime("fail/$file"));
			echo '</td>';

			//the time of the file
			echo '<td>';
				echo date("H:i:s", filemtime("fail/$file"));
			echo '</td>';

			//the filesize in KB of the file 
			echo '<td>';
				$filesize = filesize("fail/$file");
