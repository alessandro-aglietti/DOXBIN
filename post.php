...
if(file_exists("dox/DISABLEPOST")) {
...
/* Filtering out case by case spam. */

if(stripos($doxField, 'madflavortime') !== false) { die('NEVER RETURN YOU FUCKING FAGGOT SCUM. ~ Intangir'); }

if(stripos($doxField, 'nk3k2rsitogzvk2a') !== false) { die('NEVER RETURN YOU FUCKING FAGGOT SCUM. ~ Intangir'); }

if(stripos($doxField, 'trackz@exploit.im') !== false) { die('Doxbin is not your billboard, faggot. Kill yourself in the nearest fire. - nachash'); }

if(stripos($doxField, 'oa5qvgx7txodgweg') !== false) { die('NEVER RETURN YOU FUCKING FAGGOT SCUM. ~ Intangir'); }

if(stripos($doxField, 'jid.su') !== false) { die('KILL YOURSELF ASAP - nachash'); }

if(stripos($doxField, 'Starn') !== false) { die('You have updated this multiple times in one day, and I will not accept any more updates for a while. Make one big update and contact me a month from now if you want to post it. - nachash'); }
if(stripos($doxField, 'Blake Jeffrey Irving') !== false) { die('You keep causing lots of needless busy work, so posting updates to this dox has been blocked. Contact me a month from now if you want to update it. - nachash'); }

/* Blocking shit that just irks me. */

if(stripos($doxField, 'doxx') !== false) { die('You seem unable to spell dox. Come back after you pass the 2nd grade. - nachash'); }
...
if(stripos($doxField, 'Kadeem') !== false) {
    die("Quit spamming the Time Cube of dox releases already.");
}

/*if(stripos($nameField, 'Markus') !== false) {
    die("I'm sick of seeing this same dox posted repeatedly with no actual updates. Therefore, you're not allowed to post it anymore. Go spam up skidpaste or pastebin.");
}*/

/* The next 5 ifs are just to make sure that someone doesn't try to post the
filler text in the dox field. */

//if(stripos($doxField, 'Doxbin is entirely run and used by nerds who sit in their parents basements') !== false) { die('lol ur gay ~ Intangir.<script>document.location="http://edramalpl7oq5npk.onion/Offended"</script><meta http-equiv="refresh" content="2;url=http://edramalpl7oq5npk.onion/Offended">'); }

//if(stripos($nameField, 'DOXBIN') !== false) { die('lol ur still gay ~ Intangir.<script>document.location="http://edramalpl7oq5npk.onion/Offended"</script><meta http-equiv="refresh" content="2;url=http://edramalpl7oq5npk.onion/Offended">'); }
...
if(stripos($doxField, 'If you tried pasting php in this box: Nice try, hecker scum') !== false) {
    die("PUT A DICK IN YOUR EAR. - staff");
}

if(stripos($doxField, 'Try putting some actual dox in the big text box') !== false) {
    die("PUT A DICK IN YOUR EAR. - staff");
}

if(stripos($doxField, '7v4gikb4dcedx4xk') !== false) {
    die("NOT YOUR FUCKING BILLBOARD FAGGOT GO DIE IN A FIRE. - staff");
}
...
if(strlen($nameField)>30) {
   die("The name of a post cannot exceed 30 characters. Go <a href=\"index.php\">back to the index page</a>, repaste the dox, and try again with a shorter filename. Hitting your back button and trying again will just result in another error.");
}
...
if(file_exists("dox/".$nameField.".txt")) {
    die("An entry with this name <a href=doxviewer.php?dox=".$nameField.">already exists.</a>");
}
...
echo 'Dox posted. Click <a href="doxviewer.php?dox='.$nameField.'">here</a> to read them, or go <a href="index.php">back to the index</a> to post something else.';
