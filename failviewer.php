...
<a href="index.php">Post Dox</a> <a href="failviewer.php">Back to the archive</a> <a href="/faq.php">FAQ</a> <a href="proscription.php"> Proscription List</a> <a href="https://twitter.com/onetruedoxbin">Twitter</a><br />
<?php
...
header('Content-Disposition: inline; filename="failviewer.php"');
...
$filename = $_GET['fail'];
if($filename == "") { include("failarchive.php"); die(); }
...
if(file_exists('fail/'.$filename.'.txt')) {
...
unreadable, and that just won't do. */
    ...
    echo '</div><p><textarea name="failviewer" readonly="readonly" rows="25" cols="80">';
    $fail = file_get_contents('fail/'.$filename.'.txt');
    echo $fail;
    echo '</textarea></p></body></html>';

}
else {
include('failarchive.php'); // Just breaking the spaghetti into easier to manage chunks.
}
?>
